...
class FormLoader {
	...
	public function render_form()
	{
		...
		foreach ($this->form_data->fields as $field) {
			...
			if ($field->type =='element-file') {
				 $fields .= $this->element_file($field);
			}


			if ($field->type =='element-date') {
				 $fields .= $this->element_date($field);
			}
   ...
   public function render_form_embed()
	{
		if( empty($this->form_data) || empty($this->action) ) {
			throw new Exception("Error Processing Request", 1);
		}
	...
	{
		$base = base_url();
		$html = sprintf('<form action="'.$base.'%s" method="post" accept-charset="utf-8" role="form" novalidate="novalidate" enctype="multipart/form-data" >', $this->action);
	 ...
	 * @access private
	 **/
	private function element_file($field)
	{
		$id = $this->encode_element_title($field->title);
		$required = ($field->required) ? 'required' : FALSE;
		...
		$html = '<div class="form-group">';
		$html .= $this->make_label($id, $field->title, $required);
	    $html .= sprintf('<input type="file" name="%s" id="%s" class="form-control %s">', $id, $id, $required);
	  	$html .= '</div>';
     ...
     private function element_date($field)
	{
		$id = $this->encode_element_title($field->title);
		$required = ($field->required) ? 'required' : FALSE;
		...
		$html = '<div class="form-group">';
		$html .= $this->make_label($id, $field->title, $required);
	    $html .= sprintf('<input type="date" name="%s" id="%s" class="form-control %s">', $id, $id, $required);
	  	$html .= '</div>';
	...
	private function element_checkboxes($field)
	{
		...
		for($i=0; $i < count($field->choices); $i++) {
			...
			$html .= sprintf('<input type="checkbox" name="%s[]" id="%s-%d" value="%s" %s>%s', $id, $id, $i, $field->choices[$i]->value, $checked, $field->choices[$i]->title);
			$html .= '</label></div>';
		}

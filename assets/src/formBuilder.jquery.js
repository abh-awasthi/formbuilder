...
(function($) {
	...
	var baseUrl_new='http://localhost/formbuilder/';
		
		// Set default settings
		var settings = $.extend({
			load_url: '/',
			save_url: 'http://localhost/formbuilder/form/saveForm'
        }, options);
		...
		*/
		var repositionToolbox = function() {
			...
			offset = topOffset - toolboxOffset;
		 	$('#field-settings').css('margin-top', offset + 'px'); // Abhishek//
		...
		*/
		var serialize = function() {
			...
			formData['backgroundcolor'] = $('#form-backgroundcolor').val();
			formData['submitcolor'] = $('#form-submitcolor').val();
			formData['validfrom'] = $('#form-validfrom').val();
			formData['validtill'] = $('#form-validtill').val();
		...
		dust.onLoad = function(name, callback) {
		  $.ajax(baseUrl_new+'assets/src/templates/' + name + '.tpl', {
		...
		$.getJSON( settings.load_url, function( data ) {
			...
			dust.render('formbuilder-fields', base, function(err, out) {
				...
				$('.loading').fadeOut(function(){
					...
					//$('#form-backgroundcolor').val(base['form']['backgroundcolor']);
					...
					$('#save').click(function(e){
						...
						$.ajax({
							...
							console.log(response);
								var result =  JSON.parse($.trim(response));
								if (result.status==true) {
									$("#preview").attr("href","preview/"+result.slug);
                                    swal("Success!",  result.message , "success");
									window.location=baseUrl_new+"form/edit/"+result.slug
                                }else{
